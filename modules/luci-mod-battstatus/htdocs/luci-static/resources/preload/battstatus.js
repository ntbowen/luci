...
const uci = new LuCI.uci();
const displayStyle = uci.get('statusbar', 'global', 'display_style') || 'classic';
const showVoltage = uci.get('statusbar', 'global', 'show_voltage') || '0';
const showPercent = uci.get('statusbar', 'global', 'show_percent') || '1';
...
function createBatteryHTML(info) {
    ...
    let html = '<div class="battery-container" style="display: inline-flex; align-items: center; gap: 5px;">' +
               '<div class="battery-icon" style="color: ' + color + '">' +
               '<div class="battery-level' + chargingClass + '" style="width: ' + percentage + '%"></div>' +
               '</div>';
    if (showPercent === '1') {
        html += `<span class="battery-percent" style="color: ${color}">${percentage}%</span>`;
    }
    if (showVoltage === '1') {
        html += `<span class="battery-voltage" style="color: ${color}">${info.voltage}mV</span>`;
    }
    html += '<span style="margin-left: 10px;">' + currentTime + '</span>' +
            '</div>';
    return html;
}
        ...
        return callBatteryStatus().then(L.bind(function(devs) {
            ...
            for (var dev in devices) {
                if (!devs.hasOwnProperty(dev)) {
                    ui.hideIndicator('battery-' + dev.replace(" ", "-"));
                    delete devices[dev];
                }
            }
        }, this));
